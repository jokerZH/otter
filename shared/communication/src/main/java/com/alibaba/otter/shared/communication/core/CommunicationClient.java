...
public interface CommunicationClient {
    ...
    /* 指定对应的地址，进行event调用，并注册对应的callback接口. 地址格式为：127.0.0.1:1099 异步 */
    public void call(final String addr, Event event, final Callback callback);

    /* 指定对应的地址列表，进行event调用. 地址格式为：127.0.0.1:1099, 同步 */
    public Object call(final String[] addrs, final Event event);

    /* 指定对应的地址列表，进行event调用，并注册对应的callback接口. 地址格式为：127.0.0.1:1099 异步 */
